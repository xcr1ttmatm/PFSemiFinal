...
function goToMenu() {
  ...
  const orderNumber = Math.floor(Math.random() * 30) + 1;
  document.getElementById("assignedOrder").innerText = `Order number: ${orderNumber}`;
  document.getElementById("assignedOrder").dataset.orderNumber = orderNumber;
...
function printSummary() {
  const orderText = document.getElementById("assignedOrder").innerText;
  const orderNumber = orderText.replace("Order number: ", "");
  ...
  showSection("summary");
  let total = 0;
  let final = `<p><strong>Order number:</strong> ${orderNumber}</p>`;
